# ═══════════════════════════════════════════════════════════════════════
# BACKEND PATCH — add this to your existing api/user_context.py
# or paste into a new file and import it in server.py / chat.py
# ═══════════════════════════════════════════════════════════════════════

# ──────────────────────────────────────────────
# 1. ADD THIS TO /api/intake ENDPOINT
#    (extends your existing save_intake function)
# ──────────────────────────────────────────────

def save_intake_v2(guest_id: str, data: dict):
    """
    Handles both 'deep' and 'general' paths.
    'path' key tells us which flow the user took.
    """
    payload = {
        "guest_id": guest_id,
        "path":            data.get("path", "deep"),      # NEW: 'deep' or 'general'
        "q1_situation":    data.get("q1", ""),
        "q2_duration":     data.get("q2", ""),
        "q3_root_cause":   data.get("q3", ""),
        "q4_daily_impact": data.get("q4", []),
        "q5_support_need": data.get("q5", ""),
        "session_msg_count": 0,
        "is_locked": False,
        "updated_at": datetime.now(timezone.utc).isoformat()
    }
    supabase.table("user_context").upsert(payload).execute()


# ──────────────────────────────────────────────
# 2. UPDATE build_system_prompt() — ADD THIS BLOCK
#    after the existing base prompt, before returning
# ──────────────────────────────────────────────
...
def get_opening_message(ctx: dict) -> str:
    """
    Generate the AI's first message based on onboarding context.
    Called client-side via a new GET /api/opening endpoint,
    OR injected as a 'system-trigger' first message.
    """
...
def build_system_prompt_v2(guest_id: str) -> str:
    """
    Full system prompt — handles both deep and general paths.
    Drop-in replacement for your existing build_system_prompt().
    """
    ctx = get_context(guest_id)

    # ── BASE IDENTITY (shared across both paths) ──
...
"""
        ...
        q2_general = ctx.get("q2_duration", "")  # repurposed as mood
...
"""
    ...
    # Message limit awareness (shared)
...
# ──────────────────────────────────────────────
# 3. NEW ENDPOINT — Add to server.py / chat.py
# ──────────────────────────────────────────────

# @app.get("/api/opening/{guest_id}")
# async def get_opening(guest_id: str):
#     """Returns the first AI message based on onboarding context."""
#     ctx = get_context(guest_id)
#     if not ctx:
#         return {"message": "Hey — what's on your mind today?"}
#     return {"message": get_opening_message(ctx), "path": ctx.get("path", "deep")}


# ──────────────────────────────────────────────
# 4. SUPABASE MIGRATION — Run in SQL editor
#    Only adds the 'path' column if it doesn't exist
# ──────────────────────────────────────────────
MIGRATION_SQL = """
-- Add path column to user_context (safe to run on existing table)
ALTER TABLE user_context
  ADD COLUMN IF NOT EXISTS path TEXT DEFAULT 'deep';

-- Update existing rows to have path = 'deep'
UPDATE user_context SET path = 'deep' WHERE path IS NULL;
"""
